 ...
 *
 **********************************************************/
#ifndef CORE_TOOLS_LIMITS_H
#define CORE_TOOLS_LIMITS_H
...
class Polygonal3DVolume {
 public:
  ...
  bool contains(const Eigen::Vector3f &point, const float &margin)
  {
    // Point in plane frame
    std::cout << "Testing: " << point.x() << " " << point.y() << " " << point.z() << std::endl;
    std::cout << "Limits: " << height_limits.x() << " " << height_limits.y() << std::endl;
    if (point.z() < height_limits.x()+margin || point.z() > height_limits.y()-margin)
    {
      return false;
    }
    ...
    for (int i = 0; i < normals.size(); i++) {
      ...
      // Get the distance from the point to the projected point
      float dist_p_n = (v_p_c - proj_c_n * normals[i].head<2>()).norm();
      if (dist_p_n < half_sizes[i]+margin)
      {
        if ((centers[i]-point).dot(normals[i]) > 0)
        {
          return false;
 ...
 */
 public:
  Polygonal3DVolumeIterator(const Polygonal3DVolume &volume, const float &step, const float &margin)
      : volume_(volume), step_(step), margin_(margin), bounding_volume_iterator_(volume.getBoundingVolume(), step) {
  ...
  Eigen::Vector3f next() {
    Eigen::Vector3f next = current_;
    current_ = bounding_volume_iterator_.next();
    while (!volume_.contains(current_, margin_)) {
